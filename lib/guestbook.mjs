...
export async function hasMessageNameSnapshots(db) {
  try {
    await db.prepare("SELECT display_name FROM guestbook_messages LIMIT 0").all();
    return true;
  } catch { return false; }
}
...
async function isAliasUnavailable(env, displayName, ownerUserId = null) {
  const currentOwner = await env.GUESTBOOK_DB.prepare(
    "SELECT user_id FROM guestbook_users WHERE display_name = ?"
  ).bind(displayName).first();
  if (currentOwner && currentOwner.user_id !== ownerUserId) return true;
  const firstOwner = await env.GUESTBOOK_DB.prepare(
    "SELECT user_id FROM guestbook_users WHERE first_display_name = ?"
  ).bind(displayName).first();
  if (firstOwner && firstOwner.user_id !== ownerUserId) return true;
  try {
    const historicalOwner = await env.GUESTBOOK_DB.prepare(
      "SELECT user_id FROM guestbook_messages WHERE display_name = ? LIMIT 1"
    ).bind(displayName).first();
    return Boolean(historicalOwner && historicalOwner.user_id !== ownerUserId);
  } catch { return false; }
}

export async function getAvailableGuestbookAlias(env, excludedName = "", ownerUserId = null) {
  ...
  for (let attempt = 0; attempt < 256; attempt++) {
    ...
    if (displayName !== excludedName && !(await isAliasUnavailable(env, displayName, ownerUserId))) return displayName;
  }
...
export async function prepareGuestbookSubmission(request, env, requestedName) {
  ...
  if (await isAliasUnavailable(env, displayName)) {
...
export async function rerollCurrentGuestbookName(request, env) {
  const user = await findGuestbookUser(request, env);
  if (!user) {
    const error = new Error("Guestbook visitor has not registered a nickname yet");
    error.code = "not_registered";
    throw error;
  }
  const displayName = await getAvailableGuestbookAlias(env, user.display_name, user.user_id);
  let result;
  try {
    result = await env.GUESTBOOK_DB.prepare(
      "UPDATE guestbook_users SET display_name = ? WHERE user_id = ? AND display_name = ?"
    ).bind(displayName, user.user_id, user.display_name).run();
  } catch (cause) {
    if (/UNIQUE constraint failed/i.test(cause?.message || "")) {
      const error = new Error("Guestbook nickname was claimed during reroll");
      error.code = "nickname_changed";
      throw error;
    }
    throw cause;
  }
  if (result.meta?.changes !== 1) {
    const error = new Error("Guestbook nickname changed in another request");
    error.code = "nickname_changed";
    throw error;
  }
  return { token: user.token, firstDisplayName: user.first_display_name || user.display_name, displayName };
}
