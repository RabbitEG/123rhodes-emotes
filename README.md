...
**真实图库已上线。** R2 保存 4,803 张人物 crop、683 张低清预览、339 个篇目、368 位角色；公开索引和素材不提交仓库，网站通过 Pages Function binding 读取。留言会先进入私用审核页，只有手动“通过并公开”后才展示：首页显示最近 10 条，表情详情显示该 instance 最近 10 条。Cloudflare 的 D1、Turnstile 与密钥配置完成前，投稿入口保持关闭。剩余配置见 [留言板上线说明](docs/GUESTBOOK_SETUP.md)。站内匿名行为统计和私用汇总页也已实现。
