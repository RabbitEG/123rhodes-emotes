...
(async () => {
  ...
  try {
    ...
    await page.route("**/api/guestbook*", async route => {
      ...
      if (url.pathname.endsWith("/identity")) {
        if (route.request().method() === "POST") {
          registeredName = registeredName === "阿米娅#042" ? "提丰#328" : "阿米娅#042";
          return route.fulfill({ json: { enabled: true, registered: true, author_name: registeredName } });
        }
    ...
    assert.equal(await page.locator("#guestbook-reroll").isVisible(), true, "A registered visitor can reroll again");
    await page.locator("#guestbook-reroll").click();
    ...
    assert.equal(submitted.display_name, "提丰#328");
    ...
    assert.equal(await row.locator("td").nth(3).textContent(), "提丰#328", "Per-message public alias is shown separately");
    assert((await row.locator("td").nth(5).textContent()).includes("表情详情 · instance-1"));
    assert.equal(await row.locator(".moderator-status").textContent(), "待审核");
