...
class Handler(SimpleHTTPRequestHandler):
    ...
    def translate_path(self, path):
        ...
        if parts and parts[0] in ('data', 'media'):
            base = self.bundle
        elif str(relative) in ('index.html', 'search.html', 'about.html', 'privacy.html', '404.html', 'styles.css', 'background.js', 'app.js', 'stats.js', 'config/site.json', 'config/selected-background.json') or (parts and parts[0] == 'assets'):
            base = ROOT
        else:
            return str(ROOT / '__not_public__')
